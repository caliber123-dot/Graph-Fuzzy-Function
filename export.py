...
def export_images_to_excel_compare(img1, img2, output_excel="output.xlsx", max_width=500, max_height=300,fn_dict='',filetitle=''):
    """
    Export two resized PNG images to an Excel file with proper temp file handling.
    """
    temp_files = []  # To keep track of temporary files
    ...
    try:
        ...
        wb = Workbook()
        ws = wb.active
        ws.title = "BarChart_all_materials"
        ...
        # Add images to worksheet
        # ws.add_image(Image(resized_img1), "A24")
        ws.add_image(Image(resized_img2), "A11")
        data = {}        
        cutval = ""
        data['Materials'] = fn_dict['Material']
        if 'α' in fn_dict:
            data['α'] = fn_dict['α']
            cutval = "alphacut"
        if 'α′' in fn_dict:  # Fallback to 'α-α' if 'α' is missing/None
            data['α′'] = fn_dict['α′']
            cutval = "alphadash"
        ...
        # Auto-adjust only the first column (column 1 = A)
        column = get_column_letter(1)  # 'A'
        max_length = 0

        for row in ws.iter_rows(min_row=start_row, max_row=ws.max_row, min_col=1, max_col=1):
            for cell in row:
                try:
                    if cell.value:
